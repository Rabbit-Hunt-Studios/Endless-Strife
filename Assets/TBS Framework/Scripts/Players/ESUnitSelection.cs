...
namespace TbsFramework.Players
{
    public class ESUnitSelection : UnitSelection
    {
        ...
        public override IEnumerable<Unit> SelectNext(Func<List<Unit>> getUnits, CellGrid cellGrid)
        {
            ...
            {
                Unit nextUnit = null;
                var structures = units.Where(u => !alreadySelected.Contains(u) && (u as ESUnit).isStructure && u.Cell.CurrentUnits.Count == 1).ToList();
                nextUnit = units.Where(u => !alreadySelected.Contains(u) && (u as ESUnit).isStructure && u.Cell.CurrentUnits.Count == 1).FirstOrDefault();

                if (nextUnit == null)
                {
                    nextUnit = units.Where(u => !alreadySelected.Contains(u) && !(u as ESUnit).isStructure)
